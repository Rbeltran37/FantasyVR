...
public static class ParabolaPhysics {
    ...
    public static Vector3 GetParabolaInitVelocity(Vector3 from, Vector3 to, float gravity = 9.8f, float heightOff = 0.0f, float rangeOff = 0.11f) {
        ...
        DebugLogger.Info("Direction: " + direction);
        ...
        DebugLogger.Info("unitDirection: " + unitDirection);
        // Find the max height
        ...
        ft = (maxYPos - from.y) / gravity;
        if (ft < 0)
            ft = 0f;

        //float timeToMax = ft;
        float timeToMax = Mathf.Sqrt(ft);
        DebugLogger.Info("timeToMax: " + timeToMax);
        // time to return to y-target
        // Time returns to the y-axis target//

        ft = (maxYPos - to.y) / gravity;
        if (ft < 0)
            ft = 0f;

        //float timeToTargetY = ft;
        float timeToTargetY = Mathf.Sqrt(ft);
        DebugLogger.Info("timeToTargetY: " + timeToTargetY);
        ...
        DebugLogger.Info("horizontalVelocity: " + horizontalVelocityMagnitude);
        ...
        newVel.x = Mathf.Sqrt(horizontalVelocityMagnitude) * unitDirection.x;
        newVel.z = Mathf.Sqrt(horizontalVelocityMagnitude) * unitDirection.z;
        return newVel;
    }
